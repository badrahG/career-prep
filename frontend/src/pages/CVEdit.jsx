  ...
  return (
    <div className="border border-gray-200 rounded-2xl p-5 mb-4 bg-gray-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="flex items-center gap-2 mt-6"><input type="checkbox" checked={edu.currently} onChange={function(e) { onUpdate("currently", e.target.checked); }} className="w-4 h-4 accent-violet-600" /><span className="text-sm text-gray-700">Одоо сурч байгаа</span></div>
  ...
  return (
    <div className="border border-gray-200 rounded-2xl p-5 mb-4 bg-gray-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="flex items-center gap-2 mt-6"><input type="checkbox" checked={exp.currently} onChange={function(e) { onUpdate("currently", e.target.checked); }} className="w-4 h-4 accent-violet-600" /><span className="text-sm text-gray-700">Одоо ажиллаж байгаа</span></div>
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {props.items.map(function(s) {
        ...
        return <button key={s} type="button" onClick={function() { props.onToggle(s); }} className={"px-3 py-1.5 rounded-xl text-xs font-medium transition border " + (active ? "bg-violet-600 text-white border-violet-600" : "bg-white text-gray-700 border-gray-200 hover:border-gray-300")}>{s}</button>;
      })}
    </div>
  );
}
    ...
    <Layout>
      <div className="p-5 md:p-6">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 mb-5">
          ...
          <span>/</span>
          <span className="text-gray-600 font-medium">Засах</span>
        ...
        <div className="mb-5">
          ...
          <h1 className="text-xl font-bold text-gray-800">CV шинэчлэх</h1>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
          ...
          {step === 1 && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[{ id: "modern", name: "Modern", desc: "Орчин үеийн" }, { id: "classic", name: "Classic", desc: "Сонгодог" }, { id: "minimal", name: "European CV", desc: "Европ хэв маяг" }].map(function(t) {
                  var active = template === t.id;
                  return <button key={t.id} type="button" onClick={function() { setTemplate(t.id); }} className={"border rounded-2xl p-4 text-left transition " + (active ? "border-violet-400 bg-violet-50" : "border-gray-200 bg-white hover:border-gray-300")}><div className="w-full h-20 rounded-xl mb-3 bg-gray-100 border border-gray-200 flex items-center justify-center"><span className="text-xs text-gray-400">{t.name}</span></div><p className="font-semibold text-sm text-gray-800">{t.name}</p><p className="text-xs text-gray-500">{t.desc}</p>{active && <p className="text-xs text-violet-600 font-semibold mt-1">✓ Сонгосон</p>}</button>;
                })}
              </div>
              <div>
          ...
          {step === 6 && (
            <div>
              ...
              <div className="mb-6 pb-6 border-b border-gray-100"><p className="text-sm font-semibold text-gray-700 mb-3">Хувийн ур чадвар <span className="text-gray-400 font-normal">({selectedPersonal.length})</span></p><SkillToggle items={personalSkills} selected={selectedPersonal} onToggle={function(s) { toggleSkill(selectedPersonal, setSelectedPersonal, s); }} /></div>
              <div><p className="text-sm font-semibold text-gray-700 mb-3">Компьютерын програм <span className="text-gray-400 font-normal">({selectedTech.length})</span></p><SkillToggle items={techSkills} selected={selectedTech} onToggle={function(s) { toggleSkill(selectedTech, setSelectedTech, s); }} /></div>
            </div>
          )}
          ...
          {step === 7 && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-6">Товч дарж сонгоно уу.</p>
              <div className="mb-6 pb-6 border-b border-gray-100"><p className="text-sm font-semibold text-gray-700 mb-3">Мэргэжлийн ур чадвар <span className="text-gray-400 font-normal">({selectedProf.length})</span></p><SkillToggle items={profSkills} selected={selectedProf} onToggle={function(s) { toggleSkill(selectedProf, setSelectedProf, s); }} /></div>
              <div className="mb-6 pb-6 border-b border-gray-100"><p className="text-sm font-semibold text-gray-700 mb-3">Урлагийн ур чадвар <span className="text-gray-400 font-normal">({selectedArt.length})</span></p><SkillToggle items={artSkills} selected={selectedArt} onToggle={function(s) { toggleSkill(selectedArt, setSelectedArt, s); }} /></div>
              <div><p className="text-sm font-semibold text-gray-700 mb-3">Спортын ур чадвар <span className="text-gray-400 font-normal">({selectedSport.length})</span></p><SkillToggle items={sportSkills} selected={selectedSport} onToggle={function(s) { toggleSkill(selectedSport, setSelectedSport, s); }} /></div>
            </div>
          )}
              ...
              {certs.map(function(c, i) {
                return (
                  <div key={i} className="border border-gray-200 rounded-2xl p-5 mb-4 bg-gray-50">
                    <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-200"><span className="text-sm font-semibold text-gray-800">Сургалт #{i + 1}</span>{certs.length > 1 && <button type="button" onClick={function() { setCerts(removeAt(certs, i)); }} className="text-xs text-red-600 hover:text-red-700 font-medium">Устгах</button>}</div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="mt-3">
                      <label className="block text-sm font-semibold text-gray-700 mb-1.5">Гэрчилгээний файл <span className="text-gray-400 font-normal">(PDF, JPG, PNG — 10MB хүртэл)</span></label>
                      {c.file_url ? (
                        ...
                        <label className="inline-flex items-center gap-2 cursor-pointer"><span className={"px-4 py-2 text-sm rounded-xl border font-medium transition " + (certUploading[i] ? "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed" : "bg-white border-violet-400 text-violet-600 hover:bg-violet-50")}>{certUploading[i] ? "Хуулж байна..." : "+ Файл хавсаргах"}</span><input type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png" disabled={!!certUploading[i]} onChange={function(e) { if (e.target.files[0]) uploadCertFile(i, e.target.files[0]); e.target.value = ""; }} /></label>
                      )}
                    </div>
                  </div>
                );
          ...
          {step === 10 && (
            <div>
              ...
              {interns.map(function(n, i) {
                return (
                  <div key={i} className="border border-gray-200 rounded-2xl p-5 mb-4 bg-gray-50">
                    <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-200"><span className="text-sm font-semibold text-gray-800">Дадлага #{i + 1}</span>{interns.length > 1 && <button type="button" onClick={function() { setInterns(removeAt(interns, i)); }} className="text-xs text-red-600 hover:text-red-700 font-medium">Устгах</button>}</div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {step === 12 && (
            <div>
              ...
              <p className="text-sm text-gray-500 mb-6">Мэдээллээ шалгаад шинэчлэлтээ хадгална уу.</p>
              <div className="border border-gray-200 rounded-2xl p-6 space-y-5 bg-gray-50">
                <div className="flex gap-4">
                  <div className="w-20 h-24 rounded-xl border border-gray-200 bg-white overflow-hidden flex-shrink-0">{photoUrl ? <img src={photoUrl} className="w-full h-full object-cover" /> : <span className="flex items-center justify-center h-full text-gray-300 text-2xl">?</span>}</div>
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
            {step > 1 ? (
              ...
              <Link to={"/cv/" + cvId} className="px-5 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition">Цуцлах</Link>
            )}
            {step < 12 ? (
