...
function ModernTemplate({ info, educations, experiences, skills }) {
  ...
  return (
    <div style={{ background: "#fff", fontFamily: "system-ui, -apple-system, Arial, sans-serif", color: "#1f2937", minHeight: "297mm" }}>
      <div style={{ background: "linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%)", color: "#fff", padding: "28px 36px 24px" }}>
      ...
      <div style={{ padding: "24px 36px" }}>
        ...
        {experiences.length > 0 && (
          <ModernSection title="Ажлын туршлага">
            {experiences.map(function (exp, i) {
              return (
                <div key={i} style={{ marginBottom: i < experiences.length - 1 ? "12px" : 0, paddingBottom: i < experiences.length - 1 ? "12px" : 0, borderBottom: i < experiences.length - 1 ? "1px solid #f1f5f9" : "none" }}>
              ...
              return (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                  <div>
                    ...
                    {c.organization && <p style={{ fontSize: "11.5px", color: "#475569" }}>{c.organization}</p>}
                  </div>
            ...
            {languages.length > 0 && (
              <div style={{ marginBottom: "10px" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                  {languages.map(function (l, i) { return <span key={i} style={{ fontSize: "11px", background: "#f0fdf4", color: "#166534", border: "1px solid #bbf7d0", padding: "2px 8px", borderRadius: "20px" }}>{l.name}{l.level ? " · " + l.level : ""}</span>; })}
                </div>
              </div>
            )}
        ...
        {skills.length > 0 && !hasSkillSections && (
          <ModernSection title="Ур чадвар">
            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
              {skills.map(function (s, i) { return <span key={i} style={{ fontSize: "11px", background: "#eff6ff", color: "#1e40af", border: "1px solid #dbeafe", padding: "2px 8px", borderRadius: "20px" }}>{s.skill_name}</span>; })}
            </div>
          </ModernSection>
        )}
      </div>
...
function SkillGroup({ label, items, bg, color, border }) {
  return (
    <div style={{ marginBottom: "10px" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
        {items.map(function (s, i) { return <span key={i} style={{ fontSize: "11px", background: bg, color: color, border: "1px solid " + border, padding: "2px 8px", borderRadius: "20px" }}>{s}</span>; })}
      </div>
    </div>
  );
}
...
function ModernSection({ title, children }) {
  return (
    <div style={{ marginBottom: "20px" }}>
      <h2 style={{ fontSize: "13px", fontWeight: "800", color: "#1e3a8a", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "10px", paddingBottom: "5px", borderBottom: "2px solid #1e3a8a" }}>{title}</h2>
      {children}
    </div>
  );
}
...
function ClassicTemplate({ info, educations, experiences }) {
  ...
  var fullName = [info.lastName, info.firstName].filter(Boolean).join(" ");
  return (
    <div style={{ background: "#fff", fontFamily: "'Georgia', 'Times New Roman', serif", color: "#1a1a1a", minHeight: "297mm" }}>
      <div style={{ background: "#1a1a2e", color: "#fff", padding: "28px 40px 20px", textAlign: "center" }}>
      ...
      <div style={{ display: "flex" }}>
        <div style={{ width: "32%", background: "#f8f8f8", padding: "20px 16px 20px 20px", borderRight: "1px solid #e5e5e5", minHeight: "200mm" }}>
        ...
        <div style={{ flex: 1, padding: "20px 24px 20px 20px" }}>
          ...
          {certs.length > 0 && (
            <ClassicMainSection title="Сургалт, сертификат">
              {certs.map(function (c, i) {
                return (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                    <div><p style={{ fontWeight: "700", fontSize: "12px", margin: 0 }}>{c.name}</p>{c.organization && <p style={{ fontSize: "11px", color: "#555", fontStyle: "italic" }}>{c.organization}</p>}</div>
...
function ClassicSideSection({ title, children }) {
  return <div style={{ marginBottom: "16px" }}><p style={{ fontSize: "9.5px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "1px", color: "#1a1a2e", marginBottom: "6px", paddingBottom: "3px", borderBottom: "1.5px solid #1a1a2e" }}>{title}</p>{children}</div>;
}

function ClassicMainSection({ title, children }) {
  return <div style={{ marginBottom: "18px" }}><h2 style={{ fontSize: "12px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "1.5px", color: "#1a1a2e", marginBottom: "10px", paddingBottom: "4px", borderBottom: "2px solid #1a1a2e" }}>{title}</h2>{children}</div>;
}
...
function MinimalTemplate({ info, educations, experiences, skills }) {
  ...
  return (
    <div style={{ background: "#fff", fontFamily: "system-ui, -apple-system, Arial, sans-serif", color: "#111", minHeight: "297mm", padding: "40px 48px" }}>
      ...
      {certs.length > 0 && (
        <MinimalSection title="Сургалт, сертификат">
          {certs.map(function (c, i) {
            return (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                <div><span style={{ fontWeight: "600", fontSize: "12px" }}>{c.name}</span>{c.organization && <span style={{ fontSize: "11px", color: "#666" }}> — {c.organization}</span>}</div>
...
function MinimalSection({ title, children }) {
  return <div style={{ marginBottom: "20px" }}><p style={{ fontSize: "9px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "2px", color: "#888", marginBottom: "8px" }}>{title}</p>{children}</div>;
}
