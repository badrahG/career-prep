    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 relative overflow-hidden">
      ...
      <nav className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-14">
          ...
          <div className="hidden md:flex items-center gap-1 text-sm">
            ...
            <Link to="/admin/users" className="px-3 py-2 text-slate-600 hover:text-slate-900 font-medium">Хэрэглэгчид</Link>
            <Link to="/admin/cv-templates" className="px-3 py-2 text-slate-900 font-medium border-b-2 border-[#1e3a8a]">CV Загвар</Link>
          </div>
          <Link to="/admin/users" className="text-sm text-slate-600 hover:text-slate-900 font-medium">← Буцах</Link>
        </div>
      </nav>
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-6 py-3 text-xs text-slate-500 flex items-center justify-between">
          <div>
            ...
            <Link to="/admin/users" className="hover:text-slate-900">Админ</Link>
            <span className="mx-2">/</span>
            <span className="text-slate-900 font-medium">CV Загвар</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">CV Загвар удирдлага</h1>
            <p className="text-sm text-slate-600 mt-1">CV-д ашиглах загваруудыг нэмэх, засах, идэвхжүүлэх.</p>
          </div>
          <button
            onClick={openCreate}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wide px-4 py-3">Загвар</th>
                    <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wide px-4 py-3">Key</th>
                    <th className="text-left text-xs font-semibold text-slate-600 uppercase tracking-wide px-4 py-3 hidden md:table-cell">Тайлбар</th>
                    <th className="text-center text-xs font-semibold text-slate-600 uppercase tracking-wide px-4 py-3">CV тоо</th>
                    <th className="text-center text-xs font-semibold text-slate-600 uppercase tracking-wide px-4 py-3">Дараалал</th>
                  ...
                  {templates.map(function (t) {
                    return (
                      <tr key={t.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-xs font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-700">{t.key}</span>
                        </td>
                        <td className="px-4 py-3 text-sm text-slate-600 hidden md:table-cell">{t.description || "—"}</td>
                        <td className="px-4 py-3 text-center">
                          <span className="inline-block min-w-[24px] px-2 py-0.5 bg-slate-100 rounded text-xs font-semibold text-slate-700">{t.cv_count}</span>
                        </td>
                        <td className="px-4 py-3 text-center text-sm text-slate-600">{t.sort_order}</td>
                        <td className="px-4 py-3">
                          <button
                            onClick={function () { toggleActive(t); }}
                            className={"text-xs px-2 py-0.5 rounded font-medium border inline-flex items-center gap-1.5 cursor-pointer " + (t.is_active ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" : "bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100")}
                          >
                            <span className={"w-1.5 h-1.5 rounded-full " + (t.is_active ? "bg-emerald-500" : "bg-slate-400")}></span>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-1">
                            <button
                              ...
                              className="text-xs px-2 py-1 border border-slate-300 text-slate-700 hover:bg-slate-50 rounded font-medium transition"
                            >
                              Засах
                            </button>
                            <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              {modal.mode === "create" && (
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Key <span className="text-red-500">*</span></label>
                  <input
                    value={form.key}
                    onChange={function (e) { setForm({ ...form, key: e.target.value }); }}
                    placeholder="Жишээ: creative"
                    className="w-full px-3 py-2 border border-slate-300 rounded text-sm focus:outline-none focus:border-[#1e3a8a] focus:ring-1 focus:ring-[#1e3a8a] font-mono"
                  />
                  <p className="text-xs text-slate-400 mt-1">Латин үсэг, тоо, доогуур зураасыг зөвшөөрнө. Үүсгэсний дараа өөрчлөх боломжгүй.</p>
                </div>
              )}
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Нэр <span className="text-red-500">*</span></label>
                <input
                  value={form.name}
                  onChange={function (e) { setForm({ ...form, name: e.target.value }); }}
                  placeholder="Жишээ: Орчин үеийн"
                  className="w-full px-3 py-2 border border-slate-300 rounded text-sm focus:outline-none focus:border-[#1e3a8a] focus:ring-1 focus:ring-[#1e3a8a]"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Тайлбар</label>
                <input
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Урьдчилан харах зургийн URL</label>
                <input
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Дараалал</label>
                  <input
                    type="number"
                ...
                <div className="flex items-end pb-0.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-slate-700 font-medium">Идэвхтэй</span>
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button
                onClick={closeModal}
                className="px-4 py-2 text-sm border border-slate-300 rounded hover:bg-slate-50 font-medium transition"
              >
                Болих
              </button>
              <button
