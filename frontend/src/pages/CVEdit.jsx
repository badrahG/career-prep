                  ...
                  {certs.map(function(c, i) {
                    return (
                      <div key={i} className="border border-gray-200 dark:border-gray-600 rounded-2xl p-5 mb-4 bg-gray-50 dark:bg-gray-700/50">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <Inp label="Сургалт/Сайн дурын ажил *" value={c.name} onChange={function(v) { updList(certs, setCerts, i, "name", v); }} placeholder="Web Development" cls="sm:col-span-2" />
                          ...
                          {c.file_url ? (
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {profileCerts.length > 0 && (
                                <button type="button" onClick={function() { setCertPickerIdx(i); }} className="px-4 py-2 text-sm rounded-xl border font-medium transition bg-white dark:bg-gray-700 border-indigo-400 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20">🔗 Профайлаас татах</button>
                              )}
                            </div>
                          )}
                        </div>
                ...
                return (
                  <button key={pc.id} type="button"
                    onClick={function() {
                      ...
                      if (!certs[certPickerIdx].name) updList(certs, setCerts, certPickerIdx, "name", pc.title);
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {pc.issued_date && <p className="text-xs text-gray-400 dark:text-gray-500">{new Date(pc.issued_date).toLocaleDateString("mn-MN")}</p>}
                    </div>
